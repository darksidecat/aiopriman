...
class Manager:
    ...
    def get(
    ...
    ) -> partial[BaseManager[T_co, T_Storage]]:
        """
        ...
        if isinstance(man_type, str):
            if man_type in self.types.props():
                return partial(getattr(self.types, man_type), storage_data=self.storage_data)
            else:
            ...
            return partial(man_type, storage_data=self.storage_data)
        else:
